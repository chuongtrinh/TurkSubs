...
class MainWindow ( QMainWindow ):
	...
	def splitFile(self):
		...
		URLs = self.urlgen(framesdict,splits,str(self.ui.linkTextBox.toPlainText()))
		turkerPerClip = int(self.ui.numTurkerTextBox.toPlainText())
		numberOfClips = 4
		#uncomment out the line below + comment out the line above to publish all clips
		#numberOfClips = len(URLs)
		for x in range(1,numberOfClips): #a list of urls to put on mturk
			for y in range(0, turkerPerClip):
				print URLs[x]
				self.startHit(URLs[x],x)
		self.ui.plainTextEdit.setPlainText("Created Hits Successfully")
